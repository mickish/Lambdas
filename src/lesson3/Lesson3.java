...
public class Lesson3 {
   ...
   * @return
   */
  static <T> T measureOneRun(final String label, final Supplier<T> supplier) {
   ...
   */
  static <T> T measure(final String label, final Supplier<T> supplier) {
    T result = null;
   ...
   */
  static int[][] computeLevenshtein(final List<String> wordList, final boolean parallel) {
    final int LIST_SIZE = wordList.size();
    final int[][] distances = new int[LIST_SIZE][LIST_SIZE];

    IntStream stream = IntStream.range(0, wordList.size());
    if( parallel ) {
    ...
    stream
    	.forEach( n -> {
    		IntStream.range(0, wordList.size()).
    			forEach( m -> {
    				distances[n][m] = lev(wordList.get(n), wordList.get(m));
    			} );
    	}
    );
   ...
   */
  static List<String> processWords(final List<String> wordList, final boolean parallel) {
    // YOUR CODE HERE
  ...
  public static void main(final String[] args) throws IOException {
    final RandomWords fullWordList = new RandomWords();
    final List<String> wordList = fullWordList.createList(5000);
